...
const validate = {
  ruleCompleted: (rule) => !!((rule.rankFrom !== undefined && rule.rankTo !== undefined && rule.toRace !== undefined)),
  continuity: (rules) => {
    let hasError
        ...
        return <div key={'race' + I}><label>{V.nameCht}</label>
          {!V.isFinalRace && <div className={css.tableInput}>
            <table>
              ...
              <tbody>
                {render.ruleItem({races, rules: (modified && raceId === V.id) ? modified : V.advancingRules, onEdit: this.handleEdit, onRemove: this.handleRemove, raceObj: V, disabled: raceId !== V.id, options: options})}
                <tr>
